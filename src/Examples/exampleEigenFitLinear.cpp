...
//                      std::tuple<ConstraintFixedPoint<double> *> > MyWorld;
//typedef TimeStepperEigenFitLinearSMW<double, AssemblerEigenSparseMatrix<double>, AssemblerEigenVector<double>> MyTimeStepper;
typedef TimeStepperEigenFitLinearSMW<double, AssemblerParallel<double, AssemblerEigenSparseMatrix<double>>, AssemblerParallel<double, AssemblerEigenVector<double>> > MyTimeStepper;
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        EigenFitLinear *test = new EigenFitLinear(V,F,Vf,Ff,true,youngs,poisson,constraint_dir,constraint_tol, atoi(argv[5]),atoi(argv[8]),atoi(argv[9]));
    ...
    else
    {
        ...
        // no constraint switch so just create the EigenFitLinear obj with constraint switch set to 1
        // the flag indicate whether to recalculated or not
        // need to pass the material and constraint parameters to embedding too. need to do it again below. ugly
        ...
        EigenFitLinear *test = new EigenFitLinear(V,F,Vf,Ff,true,youngs,poisson,constraint_dir,constraint_tol, 1,false,10);
