...
public:
    ...
    // the fine mesh data will be used to initialize the members specific to the EigenFitLinear class
    EigenFitLinear(Eigen::MatrixXx<double> &Vc, Eigen::MatrixXi &Fc,Eigen::MatrixXx<double> &Vf, Eigen::MatrixXi &Ff, bool flag, double youngs, double poisson, int constraintDir, double constraintTol, unsigned int cswitch, unsigned int hausdorff_dist, unsigned int numModes) : PhysicalSystemImpl(Vc,Fc)
    {
        m_Vf = Vf;
        m_Ff = Ff;
...
//        std::cout<<m_coarseUs.first.transpose()*(*coarseMassMatrix)*m_coarseUs.first;
//        m_coarseMassMatrix = coarseMassMatrix;
//
