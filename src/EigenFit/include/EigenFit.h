...
//    void calculateEigenFitData(State<double> &state, MatrixAssembler &coarseMassMatrix, MatrixAssembler &coarseStiffnessMatrix,  std::pair<Eigen::MatrixXx<double>, Eigen::VectorXx<double> > &m_coarseUs, Eigen::MatrixXd &Y, Eigen::MatrixXd &Z){
    void calculateEigenFitData(const Eigen::VectorXx<double> &q, MatrixAssembler &coarseMassMatrix, MatrixAssembler &coarseStiffnessMatrix,  std::pair<Eigen::MatrixXx<double>, Eigen::VectorXx<double> > &m_coarseUs, Eigen::MatrixXd &Y, Eigen::MatrixXd &Z){
...
//                    coarse_V_disp_p = this->getImpl().getV();
...
//                        idx++;
//                    }
////                    filename_p = name_p + std::to_string(mode) + fformat;
//        //            filename_n = name_n + std::to_string(mode) + fformat;
//        //
//        //            Eigen::MatrixXi coarse_F = surftri(this->getImpl().getV(), this->getImpl().getF());
////                    igl::writeOBJ(filename_p,coarse_V_disp_p, coarse_F);
...
//            fforthogonal = (fineEig.first).transpose() * (fineEigMassProj.first);
//            cforthogonal = (m_fineP*(*N)*m_coarseP.transpose()*coarseEig.first).transpose() * fineEigMassProj.first;
//
//            fforthogonal2 = (fineEigMassProj.first).transpose() * (fineEig.first);
//            cforthogonal2 = (m_fineP*(*N)*m_coarseP.transpose()*coarseEigMassProj.first).transpose() * fineEig.first;
//
//            fforthogonal3 = (fineEig.first).transpose() * (fineEig.first);
//            cforthogonal3 = (m_fineP*(*N)*m_coarseP.transpose()*coarseEig.first).transpose() * fineEig.first;
//
//            fforthogonal4 = (fineEigMassProj.first).transpose() * (fineEigMassProj.first);
//            cforthogonal4 = (m_fineP*(*N)*m_coarseP.transpose()*coarseEigMassProj.first).transpose() * fineEigMassProj.first;
//
//            fforthogonal5 = ((*m_fineMassMatrix)*fineEig.first).transpose() * (fineEig.first);
//            cforthogonal5 = (m_fineP*(*N)*m_coarseP.transpose()*(*coarseMassMatrix)*coarseEig.first).transpose() * fineEig.first;
//
//            fforthogonal6 = (fineEig.first).transpose() * ((*m_fineMassMatrix)*fineEig.first);
//            cforthogonal6 = (m_fineP*(*N)*m_coarseP.transpose()*coarseEig.first).transpose() * (*m_fineMassMatrix)*fineEig.first;
...
//                        mode = 0;
            ...
            //            std::string coarse_name_p = "coarse_eigen_mode_p";
            //            std::string coarse_name_n = "coarse_eigen_mode_n";
            //            fformat = ".obj";
            ...
            //                if(dist_p < dist_n)
            //                {
            //                    std::cout<<"p"<<dist_p/max_scale<<std::endl;
            //
            //                } else
            //                {
